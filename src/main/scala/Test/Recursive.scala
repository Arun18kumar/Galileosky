...
object Recursive extends Galileosky {
  ...
  @tailrec
  def tagrec(tags: String): Int = {
    if (!(ptr < str_len && !flag))  1
    else {
      val tag = tags.toUpperCase().substring(ptr, ptr + 2).trim
      ptr = ptr + 2
      if (tag == "FE") {
        val ext_len = tags.substring(ptr, ptradd(2)).trim
        val milestone: Int = ptr
        mp += ("Extended tag length" -> dec(rev(ext_len)))
        ptr = ptradd(2)
        exttagrec(recstr,milestone,ext_len)
      }
      map(tag)()
      tagrec(tags)
    }
  ...
  @tailrec
  def exttagrec(extags:String, len:Int, ext:String): Int = {
    if (!(!flag && ptr - len < dec(rev(ext)) * 2)) 1
    else {
      val exttag = extags.substring(ptr, ptradd(2)).trim.replaceAll(" ", "")
      ptr = ptradd(2)
      map(exttag)()
      exttagrec(extags,len, ext)
    }
  }
